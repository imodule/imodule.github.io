                ...
                [].forEach.call(mutation.addedNodes, function (targetElem) {
                  ...
                  if (
                    [
                      ...
                      "CHECKBOX",
                      "RADIOBUTTON",
                    ].indexOf(targetElem.tagName) !== -1
                  ) {
                    reloadFlag = true;
                  }

                  !reloadFlag &&
                    [].forEach.call(childsArray, function (elem) {
                      if (
                        [
                          "SELECT",
                          "INPUT",
  ...
  Protected.prototype = {
    messages: {
      ...
      en: {
        ...
        url: {
          empty: "Please, enter your url",
          incorrect: "Incorrect url (ex: https://domain.com)",
        },
        length: {
          empty:
      ...
      !!attrName &&
        ...
        Object.keys(this.fields).forEach(function (field) {
          if (
            ...
            self.fields[field].handle[attrName] === attrValue
          ) {
    ...
    _eventSubmit: function (e) {
      ...
      //callback
      this.submitCallback(this.errors || null, this.errors ? false : true) ===
        true && this.formHandle.submit();
    },
    _eventChange: function (e) {
      ...
      if (
        ...
        new RegExp(/\s{2,}/g).test(e.target.value)
      ) {
    ...
    applySettings: function (settings) {
      ...
      // apply messages
      settings.messages &&
        Object.keys(settings.messages).forEach(function (locale) {
          Object.keys(settings.messages[locale]).forEach(function (ruleName) {
            Object.keys(settings.messages[locale][ruleName]).forEach(function (
              param
            ) {
              self.settings.messages[locale] =
                self.settings.messages[locale] || {};
              self.settings.messages[locale][ruleName] =
                self.settings.messages[locale][ruleName] || {};
              self.settings.messages[locale][ruleName][param] =
                settings.messages[locale][ruleName][param];
            });
          });
        });
    ...
    validate: function (validationField) {
      var self = this,
        fields = validationField
          ? this.getFields([validationField])
          : this.fields,
        result,
        ruleName,
        params,
      ...
      Object.keys(fields).forEach(function (n) {
        ...
        fields[n].rules &&
          Object.keys(fields[n].rules).forEach(function (ruleIndex) {
            // set rule data
            ...
            switch (fields[n].handle.type) {
              ...
              case "radio":
                ...
                if (!checked) {
                  ...
                  Object.keys(radioBtns).forEach(function (i) {
                    try {
                      message =
                        self.settings.messages[self.settings.locale][ruleName]
                          .empty;
                    } catch (e) {
                      message =
                        self.messages[self.settings.locale][ruleName].empty;
                    }
                  });
              ...
              case "file":
                ...
                if (fields[n].handle.files && fields[n].handle.files.length) {
                  ...
                  Object.keys(fields[n].handle.files).forEach(function (
                    fileIndex
                  ) {
                    switch (ruleName) {
                      case "maxfilesize":
            ...
            if (
              ...
              !(
                value === "" &&
                !fields[n].rules.join("|").match(/\|{0,1}required\|{0,1}/)
              )
            ) {
              ...
              if (result && defaultValue && value !== defaultValue) {
                ...
                result &&
                self.rules[ruleName] &&
                !self.rules[ruleName](value, params)
              ) {
    ...
    hideErrors: function (validationField, removeClass) {
      ...
      Object.keys(this.fields).forEach(function (n) {
        if (
          (validationField && validationField === self.fields[n].handle) ||
          !validationField
        ) {
          ...
          // remove class error
          removeClass &&
            self.fields[n].handle.classList.remove(
              self.settings.errorClassName
            );

          // remove error element
          errorDiv &&
            errorDiv.getAttribute("data-type") === "validator-error" &&
      ...
      var self = this,
        ...
        insertNodeError = function (refNode, errorObj) {
          ...
          if (
            ...
            refNode.nextElementSibling.getAttribute("data-type") ===
              "validator-error"
          ) {
            return;
          }
      ...
      Object.keys(this.errors).forEach(function (r) {
        ...
        if (validationField) {
          Object.keys(self.fields).forEach(function (n) {
            self.fields[n].handle.getAttribute("name") ===
              validationField.getAttribute("name") &&
              insertNodeError(self.fields[n].handle, self.errors[r]);
          });
          ...
          if (
            r === "0" ||
            (r > 0 && self.fields[r].name !== self.fields[r - 1].name)
          ) {
      ...
      this.settings.onAir &&
        Object.keys(this.fields).forEach(function (field) {
          [].forEach.call(self.settings.eventsList, function (event) {
            if (event === "keyup") {
              self.fields[field].handle[actionName](
                event,
                self._eventChangeWithDelay
              );
            } else {
